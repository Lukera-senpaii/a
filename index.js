...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN: Lukera🙈\n' 
            + 'ORG: SAYO;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=551699176633:+55 61 8149-6039\n' 
            + 'END:VCARD' 
blocked = []   
fake = 'Lukera'
prefix = '#'
...
/******** OWNER NUMBER**********/
const ownerNumber = ["551699176633@s.whatsapp.net","551699176633@s.whatsapp.net"] 
/************************************/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'Instagram:'; if (!author) author = '@luke.ra';	
...
case 'spam':
    			if (!isRegistered) return reply(ind.noregis())
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)	    				    					     	
                    if (args.length < 1) return reply(`Use assim:  exemplo : ${prefix}spam 551699176633 lindo`)
...
break
		    ...
		    ow = "551699176633@s.whatsapp.net"
		    cuy = "551699176633@s.whatsapp.net"
                    menusu = `
❏ *GRUPO MENU*
│
└◪› ${prefix}welcome [1/0]
...
break
         	   ...
         	   case 'creator':
		    		...
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)   	   
                  client.sendMessage(from, {displayname: "Lukera", vcard: vcard}, MessageType.contact, { quoted: mek})
...
break
    ...
    case 'transferi':
                ...
                addKoinUser('551699176633@s.whatsapp.net', fee)
